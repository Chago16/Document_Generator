...
<script lang="ts">
	...
	// Function to set the active link and load the page
	function setActiveLink(link: string): void {
	  if (activeLink !== link) {
		activeLink = link;
		loadPage(link);
	  } else {
		// Even if it's the same route, force reload by resetting the component
		loadPage(link);
	  }
	}
  
	// Function to dynamically import the page component based on the link
	async function loadPage(link: string): Promise<void> {
	  if (link === '/home') {
		const module = await import('./home/+page.svelte');
		CurrentPage = module.default;
	  } else if (link === '/about') {
		const module = await import('./about/+page.svelte');
		CurrentPage = module.default;
	  }
	}
  
	// Use onMount to load the page when the component is first mounted
	onMount(() => {
	  loadPage(activeLink);
	});
  </script>
...
<main>
	...
	<div class="wrapper-container">
			<div class="header">
				...
				<div class="account">
					...
					<img src="/static/icons/userIcon.png" alt="" width="46" height="46">
			...
			<div class="navigation-container">
				...
				<svg width="133" height="590" viewBox="0 0 183 836" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M0 0L134 27.5C170.5 36 183 65.8858 183 93.5V484H0V0Z" fill="#023DFE"/>
				...
				<div class="currentNav">
					<svg width="165" height="142" viewBox="0 0 165 142" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M165 0C163.935 2.99674 160.357 8.73074 155.569 10.4359C150.781 12.141 146.552 12.8125 143.348 13.1399C140.144 13.4673 46.4535 23.4536 46.4535 23.4536C4.98759 28.0322 0.254927 44.2031 0 62.9771V82.9554L165 82.9555V0Z" fill="white"/>
						<path d="M165 141.5C164.5 139 160.357 132.424 155.569 130.719C150.781 129.014 146.552 128.342 143.348 128.015C140.144 127.688 46.4535 117.701 46.4535 117.701C4.98759 113.123 0.254927 96.9518 0 78.1778V58.1995H164.971L165 141.5Z" fill="white"/>
					</svg>
				</div>
				...
				<div class="navigation-contents">
					...
					<li class:active={activeLink === '/templates'}>
						<a href="/templates" onclick={() => setActiveLink('/templates')}>
						<span class="navigation-item">
					...
					<li class:active={activeLink === '/mydocs'}>
						<a href="/mydocs" onclick={() => setActiveLink('/mydocs')}>
						<span class="navigation-item">
							<svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<li class:active={activeLink === '/details'}>
						<a href="/details" onclick={() => setActiveLink('/details')}>
						<span class="navigation-item">
							<svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	<div class="content-container">
		{#if CurrentPage}
		<svelte:component this={CurrentPage} key={activeLink} />
		{/if}
	</div>
</main>
